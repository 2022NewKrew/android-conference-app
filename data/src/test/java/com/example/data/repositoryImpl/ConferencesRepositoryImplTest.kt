...
class ConferencesRepositoryImplTest {
    ...
    fun getConferences() = runBlocking {
        ...
        assertEquals("카카오엔터프라이즈가 그려가고 있는  Enterprise IT", result?.data?.get(4)?.title)
        assertEquals("모두를 위한 서비스, 카카오가 만들어가는 세상", result?.data?.get(10)?.title)
        assertEquals("카카오 애자일 상담소 (3일차)", result?.data?.get(119)?.title)
    }
